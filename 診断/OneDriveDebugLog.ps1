...
if ($existingConnection) {
    ...
    if ($reconnect -eq "y") {
        try {
            ...
            Write-Log "接続の切断中にエラーが発生しました: $_" -LogFile $logFile -Type "ERROR" -ForegroundColor Red
        }
    }
}
...
if (-not $existingConnection) {
    ...
    try {
        ...
        Write-Log "Microsoft Graph APIへの接続中にエラーが発生しました: $_" -LogFile $logFile -Type "ERROR" -ForegroundColor Red
    }
}
