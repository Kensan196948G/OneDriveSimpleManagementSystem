# OneDriveStatusCheckLatestVersion20250227.ps1
...
try {
    ...
    function Connect-MgGraphSafely {
        ...
        try {
            ...
            Write-Host "3: 詳細認証（問題解決用）- フルスコープでの承認を要求" -ForegroundColor Cyan
            ...
            switch ($authChoice) {
                ...
                "3" {
                    # コンセント強制のためのオプション3は単にスコープを増やす形に修正
                    ...
                    $connectParams["UseDeviceAuthentication"] = $true
                    Write-DetailLog "詳細認証モードを使用します。拡張スコープでの認証を行います。" -Level INFO
                    Write-DetailLog "認証画面で「組織の管理者の承認が必要」と表示された場合は、管理者に連絡してください。" -Level WARNING
                }
            }

            # 認証を実行
            Write-DetailLog "以下の権限スコープで認証を開始します:" -Level INFO
            foreach ($scope in $connectParams.Scopes) {
                Write-DetailLog " - $scope" -Level INFO
            }
    ...
    foreach ($user in $users) {
        ...
        if ($driveInfo) {
            ...
            # 変数を安全に取得
            $quotaTotal = Get-SafePropertyValue -InputObject $quota -PropertyName 'Total'
            $quotaUsed = Get-SafePropertyValue -InputObject $quota -PropertyName 'Used'
            $quotaRemaining = Get-SafePropertyValue -InputObject $quota -PropertyName 'Remaining'
